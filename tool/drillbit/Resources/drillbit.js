...
var frontend = {
  ...
  test_status: function (name, classname) {
    ...
    el.html(classname);
    el.removeClass('untested')
      .removeClass('failed')
      .removeClass('running')
      .removeClass('passed')
      .addClass(classname.toLowerCase());
    $('#suite_' + name)
      .removeClass('suite-untested')
      .removeClass('suite-failed')
      .removeClass('suite-running')
      .removeClass('suite-passed')
      .addClass('suite-' + classname.toLowerCase());
  },
  ...
  total_progress: function (passed, failed, total) {
    $('#passed-count')
      .html('<img src="images/check_on.png"/>&nbsp;&nbsp;' + passed + ' passed');
    $('#failed-count')
      .html('<img src="images/check_off.png"/>&nbsp;&nbsp;' + failed + ' failed');
  },

  show_current_test: function (suite_name, test_name) {
    $('#current-test')
      .html('<b>' + suite_name + '</b>:&nbsp;&nbsp;' + test_name);
  },

  add_assertion: function (test_name, line_number) {
    $('#assertion-count')
      .html(Drillbit.total_assertions + ' assertions');
  },

  all_finished: function () {
    run_link_disabled = false;
    $('#current-test')
      .html('<b>Finished.</b> Took ' + Drillbit.test_duration + 's');
  }
};
...
function toggle_test_includes() {
  $.each($("div.suites img"), function () {
    if ($(this)
      ...
      .indexOf('check_on') == -1) {
      $(this)
        .attr('src', 'images/check_on.png');
    } else {
      $(this)
        .attr('src', 'images/check_off.png');
    }
  });
}
...
function clear_current_test() {
  $('#current-test')
    .html('<span style="color: #ccc">&lt;no tests currently running&gt;</span>')
}
...
function reset_all() {
  ...
  $('#assertion-count')
    .html('0 assertions');
  $('#passed-count')
    .html('<img src="images/check_on.png"/>&nbsp;&nbsp;0 passed');
  $('#failed-count')
    .html('<img src="images/check_off.png"/>&nbsp;&nbsp;0 failed');
  clear_current_test();
  Drillbit.reset();
}
...
$(window)
  ...
  Drillbit.setupTestHarness(Ti.Filesystem.getFile(Ti.Filesystem.getApplicationDirectory(), 'manifest_harness'));

  $("#test-count")
    .html(Drillbit.total_tests + ' tests in ' + Drillbit.total_files + ' files');
  ...
  for (var c = 0; c < Drillbit.test_names.length; c++) {
    ...
    suites_html += '<div class="suite" id="suite_' + name + '">' + '<img src="images/check_on.png"/> <span class="suite_name">' + name + '</span><br/>' + '<span class="description">' + entry.description + '</span></br>' + '<span id="suite_' + name + '_status" class="untested">untested</span>' + '</div>';
  }
  ...
  for (var c = 0; c < Ti.App.arguments.length; c++) {
    ...
    if (arg == '--autorun') {
      ...
      specific_tests = arg.substring(8)
        .split(',');
