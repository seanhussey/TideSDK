...
describe("UI Tests", {
  test_ui_module_methods: function () {
    ...
    value_of(Ti.UI.clearTray)
      .should_be_function();
    value_of(Ti.UI.createMenu)
      .should_be_function();
    value_of(Ti.UI.getContextMenu)
      .should_be_function();
    value_of(Ti.UI.getIdleTime)
      .should_be_function();
    value_of(Ti.UI.getMenu)
      .should_be_function();
    value_of(Ti.UI.mainWindow)
      .should_be_object();
    value_of(Ti.UI.setContextMenu)
      .should_be_function();
    value_of(Ti.UI.setDockIcon)
      .should_be_function();
    value_of(Ti.UI.setDockMenu)
      .should_be_function();
    value_of(Ti.UI.setIcon)
      .should_be_function();
    value_of(Ti.UI.setMenu)
      .should_be_function();
    value_of(Ti.UI.setBadge)
      .should_be_function();
    value_of(Ti.UI.setBadgeImage)
      .should_be_function();
  },
  test_windows_array: function () {
    value_of(Ti.UI.getOpenWindows())
      .should_be_object();
    value_of(Ti.UI.getOpenWindows()
      .length)
      .should_be(1);
    ...
    value_of(Ti.UI.getOpenWindows()
      .length)
      .should_be(2);
    value_of(Ti.UI.getCurrentWindow()
      .equals(w.getParent()))
      .should_be_true();

    var w2 = Ti.UI.getCurrentWindow()
      .createWindow('app://blahblah.html');
    value_of(Ti.UI.getOpenWindows()
      .length)
      .should_be(2);
  ...
  test_window_max_size: function () {
    ...
    value_of(w.getMaxHeight())
      .should_be(500);
    value_of(w.getMaxWidth())
      .should_be(400);
  ...
  test_window_min_size: function () {
    ...
    value_of(w.getMinHeight())
      .should_be(500);
    value_of(w.getMinWidth())
      .should_be(400);
    ...
    function openBlimpWindow() {
      if (count === 0) {
        ...
        w = Ti.UI.getCurrentWindow()
          .createWindow('app://multi_open.html');
        w.open();
        setTimeout(function () {
          closeBlimpWindow();
        }, 200);
  ...
  test_title_override_as_async: function (callback) {
    ...
    w.addEventListener(Ti.PAGE_INITIALIZED, function (event) {
      w.setTitle("blah");
    });
  ...
  test_close_message_on_originating_window_as_async: function (callback) {
    ...
    var w = Ti.UI.getCurrentWindow()
      .createWindow('app://test_close_event_listener.html');
    w.open();
  ...
  test_window_mainWnd_by_ID: function () {
    var currentWnd = Ti.UI.getCurrentWindow();
    var mainWnd = Ti.UI.getMainWindow();
    value_of(mainWnd.getID() == currentWnd.getID())
      .should_be_true();
    ...
    value_of(mainWnd == w)
      .should_be_false();
    value_of(currentWnd.getID() == w.getID())
      .should_be_false();

    var whosUrDaddy = w.getParent();
    value_of(mainWnd.getID() == whosUrDaddy.getID())
      .should_be_true();
  ...
  test_window_getchildren: function () {
    ...
    value_of(w.getChildren)
      .should_be_function();
    var children = w.getChildren();
    value_of(children)
      .should_be_object();
    value_of(children.length)
      .should_be(0);
    ...
    value_of(children.length)
      .should_be(2);
    value_of(children[0].equals(w2))
      .should_be_true();
    value_of(children[1].equals(w3))
      .should_be_true();
  ...
  test_window_opener_not_present: function () {
    ...
    value_of(window.opener)
      .should_be_null();
  },
  test_window_opener_as_async: function (callback) {
    Ti.API.ui_test_opener_value = undefined;
    var w = Ti.UI.getCurrentWindow()
      .createWindow('app://test_window_opener.html');
    w.open();
    setTimeout(function () {
      var failed = true;
      try {
        value_of(Ti.API.ui_test_opener_value)
          .should_be_true();
      } catch (e) {
        callback.failed(e);
      }
      try {
        value_of(w.window.opener)
          .should_be_object();
        failed = false;
      } catch (e) {
        callback.failed(e);
  ...
  test_yahoo_white_black_window_as_async: function (callback) {
    ...
    function openPage(page_name, run_test) {
      var w = Ti.UI.createWindow("http://api.appcelerator.net/p/pages/unittest/" + page_name + ".html");
      w.setWidth(320);
      w.setHeight(90);
    ...
    function runTest() {
      ...
      try {
        value_of(window.childWindows["black_page"])
          .should_be_object();
        value_of(window.childWindows["white_page"])
          .should_be_object();
        value_of(window.childWindows["black_page"].document)
          .should_be_object();
        value_of(window.childWindows["white_page"].document)
          .should_be_object();

        value_of(window.childWindows["black_page"].document.getElementById("button"))
          .should_be_object();
        value_of(window.childWindows["white_page"].document.getElementById("button"))
          .should_be_object();

        window.childWindows["black_page"].document.getElementById("button")
          .click();
        window.childWindows["white_page"].document.getElementById("button")
          .click();

        value_of(window.childWindows["black_page"].poke_result)
          .should_be("The White Page");
        value_of(window.childWindows["white_page"].poke_result)
          .should_be("The Black Page");
