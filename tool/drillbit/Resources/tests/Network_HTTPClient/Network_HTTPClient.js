...
describe("Network.HTTPClient", {
  before_all: function () {
    ...
    this.httpd = Ti.Process.createProcess({
      args: ['python', Ti.API.application.resourcesPath + "/httpd.py"]
    });
    this.httpd.launch();

    this.text = "here is some text for you!";
    this.reply = "I got it!";
    this.filepath = Ti.API.application.resourcesPath + "/test.txt";
  },
  ...
  test_client_properties: function () {
    ...
    var methods = ['open', 'abort', 'setRequestHeader', 'send', 'sendFile', 'getResponseHeader', 'setTimeout'];

    var props = ['readyState', 'UNSENT', 'OPENED', 'HEADERS_RECEIVED', 'LOADING', 'DONE', 'responseText', 'responseXML', 'status', 'statusText', 'connected', 'timedOut'];

    for (var c = 0; c < methods.length; c++) {
      var method = methods[c];
      value_of(this.client[method])
        .should_be_function();
    }

    for (var c = 0; c < props.length; c++) {
      var prop = props[c];
      value_of(this.client[prop])
        .should_not_be_undefined();
    }

    value_of(this.client.readyState)
      .should_be(0);
    value_of(this.client.connected)
      .should_be_false();
    value_of(this.client.timedOut)
      .should_be_false();

    value_of(this.client.UNSENT)
      .should_be(0);
    value_of(this.client.OPENED)
      .should_be(1);
    value_of(this.client.HEADERS_RECEIVED)
      .should_be(2);
    value_of(this.client.LOADING)
      .should_be(3);
    value_of(this.client.DONE)
      .should_be(4);

  },

  test_encode_decode: function () {
    var foo = Ti.Network.encodeURIComponent(null);
    value_of(foo)
      .should_be('');

    foo = Ti.Network.encodeURIComponent('');
    value_of(foo)
      .should_be('');

    foo = Ti.Network.encodeURIComponent('a');
    value_of(foo)
      .should_be('a');

    foo = Ti.Network.decodeURIComponent(null);
    value_of(foo)
      .should_be('');

    foo = Ti.Network.decodeURIComponent('');
    value_of(foo)
      .should_be('');

    foo = Ti.Network.decodeURIComponent('a');
    value_of(foo)
      .should_be('a');

    foo = Ti.Network.encodeURIComponent('a b');
    value_of(foo)
      .should_be('a%20b');

    foo = Ti.Network.decodeURIComponent(foo);
    value_of(foo)
      .should_be('a b');
  },

  test_open: function () {
    value_of(this.client.open("GET", "http://test.com"))
      .should_be_true();
    value_of(this.client.open("GET", "https://test.com"))
      .should_be_true();
    value_of(function () {
      this.client.open("GET", "bad://test.com");
    })
  ...
  test_async_get_as_async: function (callback) {
    ...
    function runTest() {
      client.addEventListener(Ti.HTTP_DONE, function () {
        try {
  ...
  test_sync_get: function () {
    ...
    this.client.addEventListener(Ti.HTTP_DONE, function () {
      done = true;
    });

    this.client.open("GET", this.url, false);
    value_of(this.client.send(null))
      .should_be_true();
    value_of(done)
      .should_be_true();
    value_of(this.client.responseText)
      .should_be(this.text);
  },
  ...
  test_timeout_as_async: function (callback) {
    ...
    this.client.addEventListener(Ti.HTTP_TIMEOUT, function (e) {
      try {
        value_of(this.timedOut)
          .should_be_true();
  ...
  test_abort_as_async: function (callback) {
    ...
    this.client.addEventListener(Ti.HTTP_ABORT, function () {
      clearTimeout(timer);
      callback.passed();
    });
  ...
  test_post_as_async: function (callback) {
    ...
    this.client.addEventListener(Ti.HTTP_DONE, function (e) {
      try {
  ...
  test_redirect_as_async: function (callback) {
    ...
    this.client.addEventListener(Ti.HTTP_REDIRECT, function () {
      try {
        value_of(this.url)
          .should_be(url);
      } catch (e) {
  ...
  test_send_cookie: function () {
    ...
    this.client.send(null);
    value_of(this.client.status)
      .should_be("200");
    value_of(this.client.responseText)
      .should_be("got the cookie!");
  },
  ...
  test_basic_auth: function () {
    ...
    value_of(this.client.status)
      .should_be("200");
    value_of(this.client.responseText)
      .should_be("authorized");
  },

  test_send_file: function () {
    var file = Ti.Filesystem.getFileStream(this.filepath);
    value_of(file)
      .should_be_object();
  ...
  test_handlers_as_async: function (callback) {
    ...
    this.client.onreadystatechange = function () {
      try {
        ...
        value_of(this.readyState)
          .should_be_number();
        value_of(this.readyState >= 1 && this.readyState <= 4)
          .should_be_true();
      } catch (e) {
        callback.failed(e);
      }
    ...
    this.client.ondatastream = function () {
      try {
        ondatastream = true;
        value_of(this.dataReceived)
          .should_be(text.length);
      } catch (e) {
        callback.failed(e);
      }
    };    
    this.client.onload = function () {
      try {
        ...
        value_of(onreadystatechange)
          .should_be_true();
        value_of(ondatastream)
          .should_be_true();
  ...
  test_get_response_header: function () {
    ...
    value_of(this.client.status)
      .should_be("200");
    value_of(this.client.getResponseHeader("Foo"))
      .should_be("Bar");
    value_of(this.client.getResponseHeader("Head"))
      .should_be("Tail");
  },
  ...
  test_http_continue_as_async: function (callback) {
    ...
    // See bug #335
    this.client.onload = function () {
      if (this.status != 200) callback.failed("status should be 200, but was: " + this.status);
      callback.passed();
    };
