...
namespace TideUtils
{
     ...
     */
    class TIDE_UTILS_API Dependency
    {
        public:
        enum Requirement
        {
     ...
     */
    class TIDE_UTILS_API KComponent
    {
        public:
        KComponentType type;
        std::string name;
    ...
    namespace BootUtils
    {
         ...
         */
        TIDE_UTILS_API int CompareVersions(std::string, std::string);

        /**
         * Compare two version strings in a piecewise way, weakly
         * @returns true if the first is larger or false otherwise
         */
        TIDE_UTILS_API bool WeakCompareComponents(SharedComponent, SharedComponent);
        ...
        TIDE_UTILS_API std::vector<std::string>& GetComponentSearchPaths();

        TIDE_UTILS_API std::vector<SharedComponent>& GetInstalledComponents(
            bool force=false);
        
        TIDE_UTILS_API SharedComponent ResolveDependency(SharedDependency dep, std::vector<SharedComponent>&);
